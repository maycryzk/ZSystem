...
import com.system.base.utils.ImageUtil;

/**
 * 网络编辑器文件图片处理
 */
@Controller
@RequestMapping("/kindEditorController/*")
public class KindEditorController {
 
    private static final ObjectMapper objectMapper = new ObjectMapper();
    private PrintWriter writer = null;
    
    private static int KEDIT_IMAGE_WIDTH = 150;
    private static int KEDIT_IMAGE_HEIGHT = 150;
    ...
    public void fileUpload(HttpServletRequest request, HttpServletResponse response)
        ...
        String savePath = application.getRealPath("/") + "inheart/resource/kindEditor/";
 
        // 文件保存目录URL
       // String saveUrl = request.getContextPath() + "/resource/kindEditor/";
        String saveUrl = "inheart/resource/kindEditor/";
...
//        List items = upload.parseRequest(request);
        ...
        MultiValueMap<String, MultipartFile> multiValueMap = defaultMultipartHttpServletRequest.getMultiFileMap();
        Iterator itr = multiValueMap.keySet().iterator();
        while (itr.hasNext()) {
        	List<MultipartFile> multipartFiles =  multiValueMap.get(itr.next());
...
//                      item.write(uploadedFile);
  				...
  				if (dirName.equals("image")) {//图片才压缩
  					File picture = new File(savePath + newFileName);
  					long size = picture.length();
  					if (size > 600 * 1024) {// 800kb以上才压缩
  						BufferedImage sourceImg = ImageIO
  								.read(new FileInputStream(picture));
  						int width = sourceImg.getWidth();
  						int height = sourceImg.getHeight();
  						if (width > KEDIT_IMAGE_WIDTH) {
  							height = height * KEDIT_IMAGE_WIDTH
  									/ width;
  							width = KEDIT_IMAGE_WIDTH;
  							ImageUtil.compressImage(savePath + newFileName,
  									savePath + newFileName,
  									KEDIT_IMAGE_WIDTH, height);
  						}
  						if (height > KEDIT_IMAGE_HEIGHT) {
  							width = width * KEDIT_IMAGE_HEIGHT
  									/ height;
  							ImageUtil.compressImage(savePath + newFileName,
  									savePath + newFileName, width,
  									KEDIT_IMAGE_HEIGHT);
  						}
  					}
  				}
                 // ImageUtil.compressImage(savePath + newFileName, savePath + newFileName, KEDIT_IMAGE_WIDTH ,KEDIT_IMAGE_HEIGHT);
...
//              }
    ...
    public void fileManager(HttpServletRequest request, HttpServletResponse response)
        ...
        String rootPath = application.getRealPath("/") + "inheart/resource/kindEditor/";
        // 根目录URL，可以指定绝对路径，比如 http://www.yoursite.com/upload/
        String rootUrl = "inheart/resource/kindEditor/";
        // 图片扩展名
